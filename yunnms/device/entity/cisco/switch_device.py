...
class SwitchDevice(Device):
    def __init__(
        ...
        super(SwitchDevice, self).__init__(
            name=name, snmp_conn=snmp_conn, ssh_conn=ssh_conn, telnet_conn=telnet_conn
        )
    ...
    def _update_interfaces_from_snmp(self) -> None:
        interfaces = []
        outputs = self._snmp_conn.get_output(
            oid=[ObjectType(ObjectIdentity("IF-MIB", "ifNumber", 0))]
        )
        inter_count = int(outputs[0][outputs[0].rindex(" ") + 1 :])
        outputs = self._snmp_conn.bulk_output(
            oid_with_NR=[
        ...
        for each in outputs[:-1]:
            ...
            output = self._snmp_conn.get_output(
                oid=[ObjectType(ObjectIdentity("IF-MIB", "ifOperStatus", snmp_index))]
            )
            status = output[0][output[0].rindex(" ") + 1 :]
            interfaces.append(
                Interface(snmp_index=snmp_index, name=name, status=status)
            )
        return interfaces
