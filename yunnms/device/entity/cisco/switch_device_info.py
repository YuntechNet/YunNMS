...
class SwitchDeviceInfo(DeviceInfo):
    ...
    def __update_from_snmp_v3(self, snmp_conn: 'SNMPv3Connection') -> None:
        ...
        self.model = re.compile(
            'Software, .*? Software').search(outputs[1]).group(0)[10:-9]
        self.hostname = outputs[0][outputs[0].rindex(' ') + 1:]
        self.version = re.compile(
            'Version .*?,').search(outputs[1]).group(0)[8:-1]
    ...
    def __str__(self):
        return 'DeviceInfo<model={}, hostname={}>'.format(
            self.model, self.hostname)
