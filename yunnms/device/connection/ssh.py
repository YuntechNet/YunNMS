...
class SSHConnection(object):
    ...
    def send_command(self, command, wrap=True, time_sleep=0.5, short=True):
        ...
        while "#" not in output and ">" not in output:
            if " --More-- " in output:
                output = output.replace(" --More-- ", "") + self.send_command(
                    command="q" if short else " ",
                    wrap=False,
        ...
        if type(commands) == list:
            ...
            for each in commands:
                output += self.send_command(
                    command=each, wrap=wrap, time_sleep=time_sleep, short=short
                )
            return output
        else:
            return self.send_command(command=commands, wrap=wrap, time_sleep=time_sleep)
